...
if [[ ${TARGET_OS} == "linux" && ${ARCH} == "x86_64" ]]; then
    ...
    cmake -S .. -B . \
        ${CMAKE_COMMON_ARG} \
        -DBUILD_TESTING=OFF -DLIBSAMPLERATE_EXAMPLES=OFF -DLIBSAMPLERATE_INSTALL=ON 2>&1 | log
    ...
    cmake -S .. -B . \
        ${CMAKE_COMMON_ARG} \
        -DBUILD_SHARED_LIBS=OFF -DBUILD_TESTING=OFF -DLIBSAMPLERATE_EXAMPLES=OFF -DLIBSAMPLERATE_INSTALL=ON 2>&1 | log
