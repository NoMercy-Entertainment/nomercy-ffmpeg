...
if [ $? -ne 0 ]; then
    echo "vulkan-headers failed to build or install"
    exit 1
fi
...
cmake -GNinja -S .. -B . \
    ...
    -DSHADERC_SKIP_TESTS=ON -DSHADERC_ENABLE_SHARED_CRT=ON 2>&1 | log
...
ninja -j$(nproc) && ninja install
if [ $? -ne 0 ]; then
    echo "shaderc failed to build or install"
    exit 1
fi
if [ ! -f "${PREFIX}/lib/pkgconfig/shaderc.pc" ]; then
    echo "shaderc.pc is missing after install"
    exit 1
fi
...
cmake -S .. -B . \
    ${CMAKE_COMMON_ARG} \
    -DSPIRV_CROSS_SHARED=OFF -DSPIRV_CROSS_STATIC=ON -DSPIRV_CROSS_CLI=OFF -DSPIRV_CROSS_ENABLE_TESTS=OFF -DSPIRV_CROSS_FORCE_PIC=ON -DSPIRV_CROSS_ENABLE_CPP=OFF 2>&1 | log
...
if [ $? -ne 0 ]; then
    echo "spirv-cross failed to build or install"
    exit 1
fi
...
ninja -j$(nproc) && ninja install
if [ $? -ne 0 ]; then
    echo "libplacebo failed to build or install"
    exit 1
fi
if [ ! -f "${PREFIX}/lib/pkgconfig/libplacebo.pc" ]; then
    echo "libplacebo.pc is missing after install"
    exit 1
fi
