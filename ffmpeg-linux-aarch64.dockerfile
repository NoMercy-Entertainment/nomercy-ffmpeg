...
COPY --from=linux /build/ffmpeg-${FFMPEG_VERSION}-linux-aarch64.tar.gz /build/ffmpeg-${FFMPEG_VERSION}-linux-aarch64.tar.gz

CMD ["sh", "-c", "cp /build/ffmpeg-${FFMPEG_VERSION}-linux-aarch64.tar.gz /output"]
